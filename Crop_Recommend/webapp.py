...
def main():  
    ...
    if st.sidebar.button("Predict"):
        if not inputs.any() or np.isnan(inputs).any() or (inputs == 0).all():
            ...
            #st.success(f"The Recommended crop is: **{prediction[0].upper()}**")
            st.markdown(f"<h3>The Recommended crop is: <span style='color: green; font-weight: bold;'>{prediction[0].upper()}</span></h3>", unsafe_allow_html=True)
